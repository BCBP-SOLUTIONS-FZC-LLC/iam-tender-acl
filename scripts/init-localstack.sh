...
#   - tenant-lifecycle-tenderacl-q   — TenantMembershipsPurged (LLD §10.1)
#   - member-removal-tenderacl-q     — MembershipRevoked (ADR-0007
#     Wave 3 Phase 3)
...
# TenantMembershipsPurged relay, and MembershipRevoked emitted from
# MembershipService.RemoveUser and ProvisioningService.DeleteMember). For
# local dev/testing, send a message directly, e.g.:
#   awslocal sqs send-message --queue-url <tenant-lifecycle queue url> \
#     --message-body '{"id":"...","type":"TenantMembershipsPurged","tenant_id":"...","time":"..."}'
#   awslocal sqs send-message --queue-url <member-removal queue url> \
#     --message-body '{"id":"...","type":"MembershipRevoked","tenant_id":"...","subject":"<user_id>","time":"..."}'
