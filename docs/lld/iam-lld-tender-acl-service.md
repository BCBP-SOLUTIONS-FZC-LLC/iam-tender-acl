...
│       ├── observability.go     -- otelSpanTracer, pgcommon.Config.Tracer's structural adapter
│       │                            (§14.3); the Logger side lives in postgres.LoggerAdapter below —
│       │                            the Zap-backed logger itself (platform-gincommon/pkg/logger)
│       │                            already matches gincommon's own Config.Logger shape directly
...
│           │                        (DSNFromEnv/ApplyStatementTimeout/MigrationDSNFromEnv, the
│           │                        single DSN-assembly path, §7.4), migrate.go (migrate.Runner
│           │                        wiring at startup), logger_adapter.go (port.Logger →
│           │                        pgcommon's Field-based domain.Logger, §14.3), migrations_fs.go
...
│                                     migrate-data-from-org-membership.sh (§21 step 1's
...
Unlike Document 3, which had to stand up `platform-pgcommon`'s tenant-context machinery against a database that had never run it before (GM-D1), this service **reuses** the identical pattern Wave 2 already built and proved (§6, §21 step 1) — the bridge itself is not new infrastructure by Wave 3. Two pools, both built via `pgcommon.NewPool`: the main, RLS-bound pool (`pgCfg, _ := pgcommon.ConfigFromEnv()` for `MaxConns`/`MinConns`/`SlowQueryThreshold`/etc., then `PGBouncerMode` forced `true` unconditionally — not env-driven — since transaction-scoped GUCs are required regardless of deployment topology, §17.5 Case 5) and a second pool on the same DSN for `processed_events` (no `GUCProvider` — that table carries no RLS, §7.3). `pgCfg.DSN` comes from `internal/adapter/outbound/postgres/db.go`'s `DSNFromEnv()`/`MigrationDSNFromEnv()` — a single DSN-assembly path (also applying `PG_STATEMENT_TIMEOUT`, when the DSN isn't a literal `DATABASE_URL`) shared with `iam-user-profile`'s and `iam-org-membership`'s identical helpers, not a second implementation hand-rolled in `cmd/tender-acl`. `withTenant` (`repository.go`) calls `pgcommon.WithValidatedGUCSet(ctx, domain.GUCSet{TenantID: tenantID.String()})` — `TenantID` only, deliberately never `UserID`/`TenantRoles`: this service's RLS policy (§7.3) filters solely on `tenant_id`, unlike Document 3's richer per-user GUC needs — then `RunInTx` wraps the transaction, binding `app.tenant_id` **transaction-locally** on every checkout so it can never persist on a pooled backend under PgBouncer transaction pooling. `IsUniqueViolation` maps TAC-2's duplicate-grant conflict to `409`; no other `pgcommon` SQLSTATE helper is used (this table has no FK/CHECK-violation case the application needs to distinguish). `migrate.Runner` (via `postgres.RunMigrations`) applies migrations against the `tender_acl_migrator` DSN. `Config.Logger` (a `postgres.LoggerAdapter` over the shared Zap-backed logger, §14.3) and `Config.Tracer` (an `otelSpanTracer` adapter) wire slow-query logging and per-query OTel spans through the same sinks as everything else in this process; `LogTenantID`/`AllowFullStatements` are left at their `false` zero-value defaults, matching every sibling's production posture. `pgmetrics` is not used — this service's own `internal/adapter/outbound/metrics` package covers every metric gincommon has no equivalent for, and pgcommon's pool-level metrics have no current consumer here. TAC-4, being a pure read with no write, is the one route where `RunInTx` wraps a `SELECT`-only transaction rather than a mutation — still GUC-bound, since the read must be tenant-scoped exactly like a write (§13.1).
...
**Explicitly out of scope for this service**, for the same structural reason §6.1/§6.3.3 already state: there is no Glue client dependency, no schema version to register, and no entry this service owns in the Glue Schema Registry, because a service that publishes zero events has no event schema to register, diff, or version. This is stated as an explicit, documented exemption rather than a silent gap — mirroring **Document 3 §7.4's** identical treatment for the same underlying reason. The only Glue-adjacent fact relevant to this service is the receiving side of §10.3: `schema-gov validate` confirms each of this service's two `receive` declarations matches the **existing** Glue-registered schema for that event (`TenantMembershipsPurged`, `MembershipRevoked`) — a read of the registry, never a write to it.
...
- **The per-user-removal cascade soft-delete (§10.1, §11.6):** idempotent by construction (`UPDATE ... SET deleted_at = now() WHERE tenant_id = $1 AND user_id = $2 AND deleted_at IS NULL`), backed by the same `processed_events` table under consumer `member_removal` — a redelivered `MembershipRevoked` is a no-op replay for the same reason TAC-2/TAC-3-style idempotency isn't needed: the `deleted_at IS NULL` predicate makes a second attempt against an already-soft-deleted row match zero rows (TAC-EVT-3).
...
| A queue delivers an `event_type` neither consumer recognizes (producer rename/add, or SNS filter-policy misroute) | `tender_acl_unexpected_event_type_total{queue,event_type}` (§14.2), `TenderAclUnexpectedEventType` ticket alert after 30 min sustained | Message is acked and dropped, never retried — no cascade runs, no `processed_events` row written | No caller-visible effect (this event type has no downstream), but a real cascade this service *should* be running may be silently skipped until the constant is updated — this is exactly how the ADR-0008 rename bug went undetected before this metric existed (`CHANGELOG.md`) |
...
- `tender_acl_member_removal_cascade_total{result}` — the §10.1/§11.6 per-user-removal cascade's outcome per processed `MembershipRevoked` event (added after this document's v2.0, **TAC-D10**, §23) — a distinct series from the tenant-offboarding one above, not a shared metric with an extra label, per TAC-EVT-6's independent-observability requirement.
- `tender_acl_unexpected_event_type_total{queue, event_type}` — incremented whenever either consumer acks-and-drops (never errors/retries) a delivery whose `event_type` doesn't match what that queue's consumer expects (§10.1/§10.5), mirroring `iam-org-membership`'s identical `iam_unknown_event_acknowledged_total`. Makes the "ignoring unexpected event type" WARN log line observable as a metric — exactly the failure mode that let both cascades silently skip-and-ack every delivery for a real stretch of time before the ADR-0008 event-type rename was caught (`CHANGELOG.md`); a sustained nonzero rate now pages via `TenderAclUnexpectedEventType` (`deploy/monitoring/app-alerts.yml`, §14.5) instead of only surfacing in logs nobody's watching.
