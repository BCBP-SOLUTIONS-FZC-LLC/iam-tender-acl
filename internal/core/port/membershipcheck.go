...
// it moved out of Core's database (docs/lld/iam-lld-tender-acl-service.md §6, §7.6.2).
...
// TAC-D11) returns it alongside active rather than a bare bool — an
// implementation MUST fail (return err) rather than return a zero
// membershipID if the provider omits it on an active:true response.
//
