...
| 2.16 | 2026-08-25 | **Two findings from a repo-alignment sweep after `iam-tender-acl` merged a batch of fixes to `main`, on explicit request to bring this document (plus `README.md`/`ARCHITECTURE.md`/`.claude/*.md`) in line with current status.** (1) **This document still named the two inbound events `TenantOffboarded`/`TenantMembershipRemoved`, which `iam-org-membership` renamed under ADR-0008 (`TenantMembershipsPurged`/`MembershipRevoked` respectively) before this document's v2.2 addition of the second event was even written — this service's own consumer code and `api/asyncapi.yaml` already shipped against the new names; only this document lagged, for the same "lives outside the repo the fix touched" reason already recorded for TAC-D10/GAP-01/GAP-02.** New decision **TAC-D12** (§23) records the catch-up explicitly rather than silently editing TAC-D7/TAC-D10's own historical decision text. Updated to the new names: **§10.1**'s queue table and mechanics bullets, **§10.3**'s `receive` operation IDs, **§10.4**'s Glue-registered schema names, **§11.6**, **§14.2**'s metric descriptions, **§18.2/§18.4**'s GDPR framing. Not touched: earlier revision-history rows above (they describe the names that were correct when each was written) and §21's Cut-over discussion of `iam-realm-provisioner`'s own, differently-scoped `TenantOffboarded` event (Discrepancy 9) — a different, unrelated event this rename does not concern. (2) **§7.4/§7.2.2 described the schema as five incremental migration files (`0001_enums` … `0005_app_role`); the repository consolidated these into one, `0001_tender_acl_schema`, since this service has never been deployed and so has no prior release's incremental history to preserve.** §7.4 gains a sentence stating this explicitly; §7.2.2's migration-filename cross-reference corrected to the new single filename. No decision register entry for this one — a packaging/tooling change, not a schema or design decision; the schema's actual shape is unchanged. |
| 2.17 | 2026-08-25 | **`IMPLEMENTATION_GAP_ANALYSIS.md` and `EVENT_COMPATIBILITY_REPORT.md` (both in the `iam-tender-acl` repo, not this document) removed from the repository, on explicit request — their discrepancy-tracking and event-invariant-verification purpose is fully absorbed into this document at this point (every discrepancy either resolved and folded into the relevant section, per 2.1–2.16 above, or explicitly still open and now tracked directly here instead), so the two standalone artifacts no longer earn their keep as separate files.** This document's own live citations of either file (§7.3, §8.4, §10.1's queue table) were updated to state the fact directly instead of pointing at a file that no longer exists. **Revision-history rows above (2.3/2.4/2.5/2.8/2.10) and TAC-D11's own decision text (§23) are deliberately left untouched** — they cite `IMPLEMENTATION_GAP_ANALYSIS.md` as the *source that surfaced* a finding at the time each was written, which remains true as a historical fact regardless of whether that file exists today; this is provenance, not a live pointer, and rewriting it would misrepresent what actually happened. No decision register entry — a documentation-set consolidation, not a design decision. |
| 2.18 | 2026-08-25 | **`MIGRATION_RUNBOOK.md` (`iam-tender-acl` repo, not this document) removed, on explicit request, following the same pattern as 2.17.** Unlike 2.17's two files, this one is a genuine, if narrow, content loss: its own Phase 1–7 breakdown carried step-level risk/rollback/observability/verification detail this section (§21) explicitly stated it "summarizes rather than duplicates" — that finer-grained detail was not migrated here and no longer exists anywhere in this repository (git history aside). §21's intro sentence corrected to stop promising detail that document no longer provides; its three dangling live citations (the intro sentence, the Cut-over step's "reality check" parenthetical, and the Rollback paragraph's snapshot-retention clause) restated to stand on their own. §6's `scripts/` tree entry's citation corrected the same way. No decision register entry — a documentation-set consolidation, not a design decision. |
| 2.19 | 2026-08-25 | **`O_AND_M_DELTA.md` (`iam-tender-acl` repo, not this document) removed, on explicit request, following the same pattern as 2.17/2.18.** Also a genuine, narrow content loss: it carried the full provider-side (`iam-org-membership`) handler-logic spec for `GET /internal/tenants/:id/members/:user_id/exists` (§4, the contract **TAC-D11** corrected §7.6.2 to match) and the Option-A/Option-B decision writeup for the per-user-removal cascade (§5, resolved by explicit user choice of Option B, **TAC-D10**) — neither is preserved elsewhere now that this file is gone; this document's own §7.6.2/§10.1/§11.6 already state the *resulting* contracts/decisions in full, just not the deliberation that produced them. Four live citations restated to stand on their own: §7.6.2's response-contract note, and three "ADR-0007 Wave 3 Phase 3 (`O_AND_M_DELTA.md` §5 Option B)" parentheticals in §10.1's queue table and §11.6. Revision-history rows 2.2/2.3 above and TAC-D10/TAC-D11's own decision text (§23) are left untouched, same provenance rationale as 2.17. No decision register entry — a documentation-set consolidation, not a design decision. |
| 2.20 | 2026-08-25 | **Logging brought in line with `iam-user-profile`'s and `iam-org-membership`'s shared convention, on explicit request: a single Zap-backed logger built via `platform-gincommon/pkg/logger.NewLogger(cfg.Environment)`, not a local `log/slog` JSON handler.** Found during the audit that prompted this: this service's own `slog.Logger`, while structurally correct (JSON, `trace_id`/`tenant_id` fields, routed into `gincommon.Config.Logger`/`pgcommon.Config.Logger`/`platform-events`' `SQSConfig.Logger` via three hand-rolled adapters), was a second, independent logging implementation and sink from the one every other current-generation sibling service uses — this section's own "identical convention to every sibling service" framing for tracing (unchanged, already correct) had drifted from being equally true for logging. New `internal/core/port/logger.go` (`Logger` — matches `platform-gincommon`'s own port shape exactly, so the shared logger satisfies it with no adapter — plus `SlogStyleLogger`, a thin wrapper preserving every existing call site's `*slog.Logger` syntax, mirroring `iam-org-membership`'s identical `port.SlogStyleLogger`, extended with a `With` method — not present there — for this service's two consumers' per-`Handle` correlation-field binding) and `internal/adapter/outbound/postgres/logger_adapter.go` (`port.Logger` → `pgcommon`'s Field-based `domain.Logger`, mirroring `iam-user-profile`'s identical `postgres.LoggerAdapter`). `cmd/tender-acl/main.go`'s three bespoke adapters (`slogDomainLogger`, `slogMapLogger` in `observability.go`, `slogPlatformLogger` in `router.go`) and its local JSON-handler `newLogger` are removed; `NewACLService`/`NewOffboardingConsumer`/`NewMemberRemovalConsumer`/`NewRouter` now take `port.SlogStyleLogger`/`port.Logger` instead of `*slog.Logger`. **`LOG_LEVEL` is removed** (`.env.example`, `docker-compose.yml`, `deploy/helm/tender-acl/values.yaml` — this document's own §15 never listed it) — neither sibling has this env var, since `platform-gincommon`'s logger has no independent level knob beyond the dev/prod preset `ENVIRONMENT` already selects (`isDevEnv`: `"dev"`/`"development"`/`"local"`). §6's package tree and §14.3 updated to describe the new files/behavior. No decision register entry — a shared-library-convention alignment, not a new design decision; behavior (structured JSON, `trace_id`/`tenant_id` correlation, `WARN`-and-swallow for best-effort cache failures) is unchanged, only the underlying implementation and sink. |
| 2.21 | 2026-08-25 | **Database connection/config/operation wiring audited against `iam-user-profile`'s and `iam-org-membership`'s identical `platform-pgcommon` usage, on explicit request — the same audit-and-fix pattern as 2.20, this time for the database layer.** Found: this service's main pool relied on `pgcommon.ConfigFromEnv()`'s own implicit DSN, while the second (`processed_events`) pool explicitly set its DSN from a bare `os.Getenv("DATABASE_URL")` — two DSN-resolution paths that happened to agree, and neither applied `PG_STATEMENT_TIMEOUT` (no pgcommon.Config field for it), unlike both siblings, which route every DSN through a shared `DSNFromEnv`/`ApplyStatementTimeout`/`MigrationDSNFromEnv` helper set. New `internal/adapter/outbound/postgres/db.go` mirrors those three functions verbatim (byte-for-byte logic, adapted doc comments); `cmd/tender-acl/config.go`'s `DatabaseURL`/`MigrationDatabaseURL` fields now come from them, and `main.go` sets `pgCfg.DSN = cfg.DatabaseURL` explicitly, matching the siblings' identical assignment (previously implicit). **Everything else audited was already correct, no change needed**: `wrapConnErr` already matched `iam-user-profile`'s identical helper almost verbatim; `withTenant`'s `pgcommon.WithValidatedGUCSet(ctx, domain.GUCSet{TenantID: ...})` (never `UserID`/`TenantRoles`) is correct and intentional — this service's RLS policy (§7.3) only ever filters on `tenant_id`, unlike the siblings' richer per-user GUC needs; `SystemDSNFromEnv` (the siblings' BYPASSRLS cross-tenant pool for admin/reconciler jobs) has no analog here and isn't one — the `processed_events` second pool is same-role, no-RLS-table-only, a different need with nothing to import; `/readyz`'s `Pool.Health(ctx)`, `migrate.Runner` wiring, and `DrainAndClose` shutdown were already aligned from earlier passes (2.9 and prior `CHANGELOG.md` entries). **§6.3.2 rewritten** — found, while writing this entry, to already be stale independent of anything this pass changed: it described hardcoded example `MinConns: 0, MaxConns: 10` (actual code reads both from `pgcommon.ConfigFromEnv()`), an unvalidated `pgcommon.WithGUCSet(ctx, GUCSet{UserID, TenantID, TenantRoles})` call binding fields this service never sets (actual code is `WithValidatedGUCSet` with `TenantID` only, matching this table's tenant-only RLS policy), and `IsCheckViolation`/`IsForeignKeyViolation`/`pgmetrics.Init` usage this codebase has never had (only `IsUniqueViolation`, for TAC-2's duplicate-grant conflict, is used) — corrected to describe the real wiring, including the new `db.go` helpers from this pass. No decision register entry — a shared-library-convention alignment, not a new design decision; DSN values are unchanged in every environment that doesn't set `PG_STATEMENT_TIMEOUT` (a new, previously-unavailable knob, not a behavior change to anything already configured). |
| 2.22 | 2026-08-25 | **Events/outbox/dedup wiring audited against `iam-user-profile`'s and `iam-org-membership`'s identical `platform-events` usage, on explicit request — the same audit-and-fix pattern as 2.20/2.21, this time for the event layer.** Outbox: not applicable — this service publishes zero events by design (TAC-EVT-1, TAC-D5) and has no analog to audit against the siblings' `outbox.ApplySchema`/`outbox.NewRunner`. Idempotency/dedup: this service's check-before/mark-after `processed_events` pattern already matches `iam-user-profile`'s (decoupled mark-after-success); `iam-org-membership`'s richer `MarkProcessedInTx` (dedup write committed atomically with the state-changing write, in the same transaction) is a deliberately heavier pattern justified by its own more complex event-ordering/staleness guards (EVT-14/15) that don't apply here — this service's two cascades are idempotent by construction regardless of ordering, so the simpler decoupled pattern is correct as-is, not a gap. SQS consumer construction (`WithConcurrency`, no `WithDeadLetterHandler`/`WithMaxReceiveCount`, redrive left to each queue's own SQS policy) already matches both siblings' identical choices. **One real gap found**: neither consumer had any metric for a delivery whose `event_type` doesn't match what that queue expects — only a WARN log line — unlike `iam-org-membership`'s `iam_unknown_event_acknowledged_total{topic,event_type}`, which is exactly the observability that would have caught the ADR-0008 event-rename bug this service already suffered (both cascades silently skip-acking every delivery for a real stretch of time, `CHANGELOG.md`) far sooner than it actually was caught. New `tender_acl_unexpected_event_type_total{queue,event_type}` (§14.2), incremented at the same point both consumers already log the WARN; new `TenderAclUnexpectedEventType` ticket alert (§14.5, `deploy/monitoring/app-alerts.yml` + the Helm chart's `templates/prometheusrule.yaml`), mirroring `iam-org-membership`'s identical `IAMUnknownEventType` (severity `ticket`, `for: 30m`, 5m evaluation). New §12.3 failure-scenario row for the same case. While in `deploy/monitoring/app-alerts.yml`/`templates/prometheusrule.yaml`/`slo-rules.yml`, found and fixed unrelated staleness from the ADR-0008 rename (2.16 fixed this document and the `.claude`/root docs but never touched `deploy/monitoring/`): alert/SLO description text still said `TenantOffboarded`/`TenantMembershipRemoved` — corrected to `TenantMembershipsPurged`/`MembershipRevoked` (label matchers/metric names were already correct — this service's own `tender_acl_*` cascade-metric names never carried the event name). No decision register entry — an observability addition plus doc-drift corrections, not a design decision. |
| 2.23 | 2026-09-09 | **Logs/metrics/traces composition-root order brought in line with `iam-org-membership` and `iam-realm-provisioner`, on explicit request.** Found: tracing was installed lazily inside `NewRouter`'s `ObservabilityMiddlewares` (so migrations, pool connect, and any span started before the HTTP engine got a noop provider); business collectors were registered *before* gincommon's metrics-init API ran (so they missed `{service, version}` const labels); `gincommon.Shutdown(nil)` skipped the Zap flush; `TimeoutMiddleware` was documented in §6.3.1 but not wired. `cmd/tender-acl/main.go` now calls `gincommon.InitTracing` then `ObservabilityMiddlewares` before any collector registration; business/`platform-events` collectors inherit gincommon's registerer and const labels; shutdown flushes Zap; `TimeoutMiddleware(30s)` sits in front of ObservabilityMiddlewares. `cmd/tender-acl/observability.go`'s `otelSpanTracer` moved to `internal/adapter/outbound/postgres/otel_tracer.go` (`NewOTelTracer`) matching both siblings. No decision register entry — a shared-library-convention alignment; metric *names* are unchanged (only const labels added). |
| 2.24 | 2026-09-09 | **Database connection/config/operation wiring brought in line with `iam-org-membership` and `iam-realm-provisioner`'s `platform-pgcommon` usage, on explicit request — the same audit-and-fix pattern as 2.21/2.23, this time closing the remaining pgcommon gaps.** Found: pin was `v1.2.1` (siblings `v1.3.0`, so `IsConnectionException`/`IsInsufficientResources`/`IsPgError` were unavailable); `pgmetrics` was unused; the `processed_events` pool was a bare `pgcommon.Config{DSN, Logger, Tracer}` literal that dropped `ConfigFromEnv` pool sizing/`SlowQueryThreshold`; `wrapConnErr` remapped every unrecognized Go error to `dependency_unavailable` (the bug both siblings already fixed — a business error from inside `RunInTx` became a misleading 503); `ApplyStatementTimeout` was not idempotent and skipped `MIGRATION_DATABASE_URL`; Helm had no `PG_STATEMENT_TIMEOUT`. `platform-pgcommon` bumped to **v1.3.0**. `pgmetrics.InitWithRegisterer` shares gincommon's registerer with events/business collectors. New `LedgerPoolConfig` copies the siblings' `SystemPoolConfig` pattern (ConfigFromEnv sizing, no `GUCProvider`, `PGBouncerMode` forced true) for the ledger pool — not a BYPASSRLS sysPool; this service has no reconciler/`SYSTEM_DATABASE_URL`. `wrapConnErr` remaps only SQLSTATE 08/53/57/58 and `puddle.ErrClosedPool`. `ApplyStatementTimeout` is idempotent and applied to the migrator DSN. Both pools `defer Close()` as a safety net after `DrainAndClose`. Helm/`PG_STATEMENT_TIMEOUT` defaults to `5s`. No decision register entry — a shared-library-convention alignment; no new domain behavior. |
| 2.25 | 2026-09-09 | **Remaining `platform-pgcommon` operation/config gaps vs `iam-org-membership` / `iam-realm-provisioner` closed, on explicit request.** `withTenant` now uses `RunInTxWithRetryOpts` (deadlock/serialization retry, same `writeRetryOpts` as both siblings). `wrapConnErr` also maps Go-level network errors (`isNetworkError`, realm-provisioner). `respondACLError` reclassifies a leaked raw PgError (SQLSTATE 08/53/57/58) to 503, matching both siblings' `HandleError` fallback. `PG_MIN_CONNS` forced `0` in code and helm under transaction pooling. No decision register entry. |
| 2.26 | 2026-09-09 | **Events/outbox/dedup wiring brought in line with `iam-org-membership` / `iam-realm-provisioner`'s `platform-events` usage, on explicit request — the same audit-and-fix pattern as 2.22/2.24/2.25.** Outbox: still not applicable (TAC-EVT-1, TAC-D5) — `outbox.ApplySchema`/`outbox.NewRunner`/`events.NewSNSPublisher` are never called. Found: queue #2 was a hand-rolled `events.SQSConfig` literal; `processed_events` lived in the inbound consumer package on a second no-GUC pool, with check-then-mark *after* the cascade rather than in the same transaction; unknown types were acked without a ledger row (redelivery storms the same unknown type); no `processed_events_duplicates` metric. Both queues now go through `SQSConfigFromEnv`/`SQSConsumerOptions` (queue #2 clones `LoadSQS` and overrides URL + concurrency). `ProcessedEvents` moved to the postgres adapter on the app pool; `port.TxRunner` + `withPool` so cascade + `MarkProcessed` share one commit (IDEMP-2); `skipDuplicate`/`ackUnknown` match both siblings; new `tender_acl_processed_events_duplicates_total{consumer}`. No decision register entry — a shared-library-convention alignment. |
| 2.28 | 2026-09-10 | **Production-readiness audit and remediation pass, on explicit request ("fix all" findings from a three-dimension code audit: core domain/service/repository/consumer logic, HTTP/API/security/config, tests/CI/deploy).** New decision **TAC-D13** (§26) records the one genuine design-level addition, TAC-1 pagination; every other finding was a plain bug/tooling fix, not a design decision. **Code fixes:** `membershipcheck.HTTPChecker.Exists` now fails closed (returns an error) on an `active:true` response missing `tenant_membership_id` instead of silently writing a placeholder zero UUID into that `NOT NULL` column (§7.6.2's contract, TAC-D11, now actually enforced — previously specified correctly here but not enforced in code); TAC-1 (list) is now paginated (§8.4 gains a **new TAC-1 subsection**; §23.2 rewritten — its prior "unbounded and unpaginated, by design" framing is corrected, since an unbounded response was a real gap, not merely a theoretical one; TAC-D13); a new non-partial index `idx_tae_tenant_all` (§7.2) closes a missing-index gap on the tenant-offboarding cascade delete; TAC-2/TAC-3 gained a request-body size cap (defense-in-depth, no document change needed — an implementation-only hardening); `getEnvInt` now fails startup on a malformed (not just missing) env var instead of silently defaulting; `DOCS_ENABLED` parsing is now case-insensitive. **Infrastructure fixes (no LLD section affected, `CHANGELOG.md` has the detail):** a broken Docker build (`.dockerignore` excluded `docs/` wholesale despite `main.go` blank-importing `docs/swagger`) that would have failed on every `docker build` of this service — found and fixed, then verified with a real build and boot against real Postgres/Valkey/LocalStack; two silent-pass holes in `release.yml`'s deploy-gate; a real HTTP smoke test added to CI; the coverage gate raised from 70% to 85% (real merged coverage was already 91.6% at the time of the raise). **Documentation-only fixes, same "lives outside/adjacent to the repo the fix touched" class as TAC-D12/GAP-01/GAP-02:** stale pre-ADR-0008 event names in `scripts/init-localstack.sh`, Helm `values.yaml`, `.env.example`, and several `.github/` files (CODEOWNERS, PR/issue templates, SECURITY.md); `.github/CODEOWNERS` also referenced `O_AND_M_DELTA.md`/`MIGRATION_RUNBOOK.md`, both removed per this document's own 2.18/2.19 — repointed to this document's Migration Plan section (§24), the documented successor. All Go changes verified: `go build`/`go vet`/`gofmt` clean; unit, integration (testcontainers Postgres/Valkey/SQS-compatible), rls, and e2e suites all pass; `make swag` regenerated to match the new TAC-1 query params. |
| 2.27 | 2026-09-10 | **Structural alignment pass against `iam-org-membership`'s own current LLD (`iam-lld-org-membership-service.md`), on explicit request — that document (this document's original v2.0 structural template, alongside `iam-lld-user-profile v1.md`) has grown three top-level sections and two subsections since this document's own v2.0 pass adopted its section template (**TAC-D9**, §26), and this document had not kept pace.** Two genuine subsection gaps closed, both additive, offset by this document's own §1–§3 preamble (§4 note): **§9.5 Cache invariants** (new — mirroring the org LLD's own §6.5, adapted to this service's single `tac:acl` cache region; grounded directly against `internal/adapter/outbound/valkey/cache.go` and `internal/core/service/acl_service.go` rather than copied from the org LLD's own cache invariants, since this service has no read-through/seat-usage/jitter analogs to theirs) and **§10.2 Serialization format** (new — mirroring the org LLD's own §7.2; this service's own consume-only JSON posture, already stated piecemeal in §10.1, given its own subsection) — the latter insertion shifts the former §10.2–§10.5 (Outbound, AsyncAPI contract, Glue Schema Registry, Event invariants and audit) down one position each, to **§10.3–§10.6**; every cross-reference into old §10.2–§10.5 elsewhere in this document was updated to match, following this document's own established v2.13 renumbering precedent. Three entirely new top-level sections added, mirroring the org LLD's own §18/§20/§21, none of which this document had any analog for: **§21 Integration Details** (per-integration subsections for Core, the Tender Service, and AuthZ Enrichment, plus a cross-service dependency table — this service's much smaller integration surface than Core's own, by design, §3/§25), **§22 Operational Considerations** (outbox health stated N/A, RLS violation monitoring, the grant-time membership-check dependency's health, a cross-service degradation matrix, and the tenant/member cascade fan-out health note), and **§23 Performance Considerations** (the TAC-4 hot path, TAC-1's unbounded/unpaginated-by-design list query — verified directly against `internal/adapter/outbound/postgres/repository.go`'s actual `List` query, not assumed — and the single-row TAC-2/TAC-3 write path). The former **§21 Migration Plan**, **§22 Future Option**, and **§23 Decision Register** shift accordingly to **§24, §25, §26**; every cross-reference to the old numbers throughout this document (the §4.1 table, §5.3, §6, §6.2, §6.3.2, §7.4, §19, §24's own Cut-over text, and every `TAC-D#`/`TAC-Q#` entry citing them) was updated to match, exhaustively re-`grep`-checked after the edit. The Table of Contents was updated to list all three new sections and all six renumbered ones. **No existing content, decision, or invariant was removed — every addition is new material or a relocated cross-reference**, following this document's own "extend, don't destroy" precedent (esp. v2.13, v2.17–v2.19). Sections legitimately left as-is after comparison, because the divergence from the org LLD is service-specific rather than a gap: §5.3 (this service's own interim-vs-authorization-decision-consumer framing vs. org's delegation/availability ownership split), §7 (org's richer four/five-table data model vs. this service's single table), §11 (each document's own distinct request flows), §18/§18 GDPR (org's trial/reactivation/paid-offboarding subsections have no analog here — this service has no trial concept), and §19 (this document's Open Questions register already used the org LLD's own Group A/B/C/D convention, confirmed unchanged). No decision register entry — a structural/documentation-completeness addition, matching this document's own precedent at v2.13/v2.17–v2.19. |
...
21. [Integration Details](#21-integration-details)
22. [Operational Considerations](#22-operational-considerations)
23. [Performance Considerations](#23-performance-considerations)
24. [Migration Plan](#24-migration-plan)
25. [Future Option: Moving into the Tender Service (Wave 4)](#25-future-option-moving-into-the-tender-service-wave-4)
26. [Decision Register](#26-decision-register)
...
- **Not** an events producer for ACL activity. No grant/revoke event exists in the HLD's event catalogue today, and this extraction does not introduce one (§10.3).
...
|---|---|---|
| ADR-0007 (Document 1) | Wave sequencing, cross-service FK-loss rationale, Action Items 7–8 | §24, §25 |
...
| Audit records | Audit Log | Append-only compliance store; this service's writes are locally audit-logged per §10.6, not stored as a second copy here |
...
Every sibling extraction in this decomposition reduces to a single distinguishing question: does the extracted service sit on a live authorization decision, or does it only store configuration another service applies? Document 2's Catalog Service and Document 3's Group Mapping Service both answer "configuration only" — neither is ever consulted synchronously to decide whether a specific request should be allowed. This service answers differently: TAC-4/I-12 **is** the authorization answer the Tender Service's approval workflow consumes directly (§11.4). This is why this service, alone among the three waves, treats read-path latency and cache-failure behaviour (§9, §14.1) with the same rigor a hot-path service would, even though its write volume is low and its footprint is deliberately minimal (§25) — the service is small, but the one read it serves is load-bearing for a real access decision, not advisory configuration.
...
**History:** this service originally shipped (v1.0/v2.0 of this document) as a deliberately flat, near-literal lift of the source LLD's P-21/22/23/I-12 handlers, per ADR-0007's explicit instruction — the reasoning at the time was that a service scheduled for a Wave 4 merge (§25) should not pay for internal abstraction boundaries it will only have to unwind again. That decision (**TAC-D1**, §26) was later reversed on explicit request, to bring this service's tooling (`go-arch-lint`) and mental model in line with every sibling repo — see `CHANGELOG.md` for the restructuring commit. TAC-D1 is marked **superseded** in §26 rather than removed, so the original reasoning isn't lost. The interim/disposable framing itself (§25) is unchanged by this — only the package structure.
...
│       │                            HTTP router; self-migrates at startup. Bootstraps logs/
│       │                            metrics/traces through platform-gincommon (InitTracing +
│       │                            ObservabilityMiddlewares) before any collector registration,
│       │                            matching iam-org-membership / iam-realm-provisioner
...
│   │   ├── port/                -- TenderACLRepository · MembershipCheckClient · Cache · TxRunner (interfaces
│   │   │                            only) · Logger/SlogStyleLogger (§14.3 — the one port with a real
│   │   │                            implementation here: a thin *slog.Logger-call-syntax wrapper
│   │   │                            over the shared Zap-backed logger, not an adapter to a
...
│       │   │                        asyncapi.go (the §10.4 HTML viewer) — TAC-1/2/3 (public,
│       │   │                        role-gated), TAC-4 (internal, mesh-only, never 404)
│       │   └── consumer/        -- OffboardingConsumer + MemberRemovalConsumer + skipDuplicate/ackUnknown
│       │                            — tenant-lifecycle-tenderacl-q (§10.1) and
│       │                            member-removal-tenderacl-q (ADR-0007 Wave 3 Phase 3)
│       └── outbound/
│           ├── postgres/        -- TenderACLRepository implementation (pgx/pgcommon), db.go
│           │                        (DSNFromEnv/ApplyStatementTimeout/MigrationDSNFromEnv/
│           │                        LedgerPoolConfig/TxRunner/withPool/wrapConnErr, the single DSN-assembly and
│           │                        SystemPoolConfig-pattern helper, §6.3.2/§7.4), processed_events.go
│           │                        (idempotency ledger joining ambient TxRunner tx, IDEMP-2), migrate.go
│           │                        (migrate.Runner wiring at startup), logger_adapter.go
│           │                        (port.Logger → pgcommon's Field-based domain.Logger, §14.3),
│           │                        otel_tracer.go (postgres.NewOTelTracer — pgcommon.Config.Tracer,
│           │                        same helper iam-org-membership / iam-realm-provisioner use),
│           │                        migrations_fs.go (//go:embed's migrations/*.sql — same
│           │                        "no on-disk source tree in the distroless image" rationale
│           │                        as api/embed.go below), migrations/
...
├── api/
│   ├── asyncapi.yaml             -- two receive-only operations (§10.1/§10.4) — no send operations
│   └── embed.go                 -- //go:embed's asyncapi.yaml into the binary at compile time, so
│                                     GET /asyncapi (§10.4) needs no file present at runtime — the
...
│                                     migrate-data-from-org-membership.sh (§24 step 1's
│                                     export/replication tooling),
...
- `internal/core/service` depends only on `domain`+`port` — `ACLService.Grant` calls `port.MembershipCheckClient`'s **interface**, never `internal/adapter/outbound/membershipcheck`'s HTTP implementation directly. This is precisely the boundary the Wave 4 merge needs to repoint or delete cheaply (§7.6.2, §25).
...
- Enforced by `go-arch-lint` in CI (`.go-arch-lint.yml`), identically to every sibling service — this supersedes the single interface-boundary check the original flat layout enforced (§26, TAC-D1).
...
Unlike Document 3, which had to stand up `platform-pgcommon`'s tenant-context machinery against a database that had never run it before (GM-D1), this service **reuses** the identical pattern Wave 2 already built and proved (§6, §24 step 1) — the bridge itself is not new infrastructure by Wave 3. One pool, built via `pgcommon.NewPool`: the RLS-bound app pool (`pgCfg, _ := pgcommon.ConfigFromEnv()` for `MaxConns`/`MinConns`/`SlowQueryThreshold`/etc., then `PGBouncerMode` forced `true` unconditionally — not env-driven — since transaction-scoped GUCs are required regardless of deployment topology, §17.5 Case 5). `processed_events` shares this pool so a cascade write and `MarkProcessed` can join one `TxRunner` transaction via `withPool` (IDEMP-2); that table has no RLS, so an INSERT inside a GUC-bound tx is fine. `LedgerPoolConfig` remains the `SystemPoolConfig` pattern from `iam-org-membership` / `iam-realm-provisioner` (ConfigFromEnv sizing, clear `GUCProvider`, force `PGBouncerMode` true) for tests / a future no-GUC pool — not a BYPASSRLS sysPool; this service has no reconciler. `pgCfg.DSN` comes from `internal/adapter/outbound/postgres/db.go`'s `DSNFromEnv()`/`MigrationDSNFromEnv()` — a single DSN-assembly path (also applying `PG_STATEMENT_TIMEOUT`, idempotently, including onto `MIGRATION_DATABASE_URL`) shared with `iam-user-profile`'s and `iam-org-membership`'s identical helpers, not a second implementation hand-rolled in `cmd/tender-acl`. `withTenant` (`repository.go`) calls `pgcommon.WithValidatedGUCSet(ctx, domain.GUCSet{TenantID: tenantID.String()})` — `TenantID` only, deliberately never `UserID`/`TenantRoles`: this service's RLS policy (§7.3) filters solely on `tenant_id`, unlike Document 3's richer per-user GUC needs — then joins an ambient `TxRunner` tx when present, otherwise `RunInTxWithRetryOpts` wraps the transaction (deadlock/serialization retry, same `writeRetryOpts` as both siblings), binding `app.tenant_id` **transaction-locally** on every checkout so it can never persist on a pooled backend under PgBouncer transaction pooling. `IsUniqueViolation` maps TAC-2's duplicate-grant conflict to `409`; `wrapConnErr` remaps SQLSTATE class 08/53/57/58 and a closed pool to `dependency_unavailable` via `IsConnectionException`/`IsInsufficientResources`/`IsPgError` (v1.3.0) — unrecognized business errors from inside `RunInTx` pass through. `migrate.Runner` (via `postgres.RunMigrations`) applies migrations against the `tender_acl_migrator` DSN. `Config.Logger` (a `postgres.LoggerAdapter` over the shared Zap-backed logger, §14.3) and `Config.Tracer` (`postgres.NewOTelTracer`, the same helper `iam-org-membership` / `iam-realm-provisioner` use) wire slow-query logging and per-query OTel spans through the same sinks as everything else in this process; `LogTenantID`/`AllowFullStatements` are left at their `false` zero-value defaults, matching every sibling's production posture. `pgmetrics.InitWithRegisterer` registers pool/query collectors on `gincommon.MetricsRegisterer()`, matching both siblings. TAC-4, being a pure read with no write, is the one route where `RunInTx` wraps a `SELECT`-only transaction rather than a mutation — still GUC-bound, since the read must be tenant-scoped exactly like a write (§13.1).

#### 6.3.3 `platform-events` — SQS consumer only, no outbox, no publisher

Identically to Document 3's Group Mapping Service (§7.3.3 there), this service's `platform-events` usage is **strictly inbound**: two active subscriptions — `tenant-lifecycle-tenderacl-q` and `member-removal-tenderacl-q` (added after this document's v2.0, **TAC-D10**, §26) — each its own `events.NewSQSConsumer` call; idempotency via the same shared `processed_events` table for both, discriminated by consumer name, identical shape to every other IAM consumer. Both queues' config goes through `platform-events/pkg/config`'s `LoadSQS`/`SQSConfigFromEnv`/`SQSConsumerOptions` env-var helpers: `tenant-lifecycle-tenderacl-q` uses `LoadSQS` directly; `member-removal-tenderacl-q` clones that env and overrides `QueueURL` + concurrency (`MEMBER_REMOVAL_SQS_CONCURRENCY`, falling back to `CONSUMER_CONCURRENCY`) because `LoadSQS` has no concept of a second queue. Known-type handlers run `skipDuplicate` then one `TxRunner` transaction for the cascade write and `MarkProcessed` (`withPool` / IDEMP-2, matching `iam-org-membership` / `iam-realm-provisioner`). Unknown types go through `ackUnknown` (metric + `MarkProcessed`) so redelivery does not storm. `platform-events` has no processed-events API — `Envelope.ID` + `INSERT ON CONFLICT DO NOTHING` is the library's documented consumer pattern. **`outbox.ApplySchema`, `outbox.NewRunner`, and `events.NewSNSPublisher`/`NewRoutingPublisher` are never called** — there is no `outbox_events` table, no publisher wiring, and no outbound eventbus adapter package, because this service produces no event of its own (§10.3, TAC-EVT-1).
...
**`idx_tae_tenant_all` is deliberately non-partial**, unlike the three indexes above it — added after a production-readiness audit found the tenant-offboarding cascade delete (§7.6.4/§11.5, `CascadeDeleteForTenant`) had no usable index: it deletes **every** row for a tenant, including already-soft-deleted ones retained for audit (§18), so a `WHERE deleted_at IS NULL` partial index can't cover it. Without this index, a tenant with any revoke history forced a sequential scan under the cascade's write transaction.
...
**Response contract note — this section's `{"active": true|false}` shape (pre-2.3 revisions of this document) is insufficient by construction, not merely by this implementation's particular design choice.** `tender_acl_entries.tenant_membership_id` is `NOT NULL` (§7.2.1), carried over unchanged from the source schema. The composite FK this check replaces is precisely what used to guarantee a valid `tenant_membership_id` existed to reference (§7.6.1) — a bare boolean gives this service no other way to populate that column at grant time. The response therefore **must** carry `tenant_membership_id` alongside `active` whenever `active` is `true`; it is correctly omitted (or `null`) whenever `active` is `false`, since there is then no membership row to reference. This is a required correction to this document, not an optional extension — see **TAC-D11** (§26) for the correction's own history. The full provider-side handler logic (`iam-org-membership`'s `GET /internal/tenants/:id/members/:user_id/exists`) this contract specifies is `iam-org-membership`'s own implementation detail, not this document's to carry.
...
#### TAC-1 — `GET /api/v1/tenants/:id/tenders/:tender_id/acl`

**Paginated (TAC-D13, §26).** Query params `limit` (default 100, hard ceiling 500 — a caller-requested value above the ceiling is silently clamped, not rejected) and `offset` (default 0), both validated: a non-positive/non-integer `limit` or a negative/non-integer `offset` returns `400 invalid_request`. Response echoes the applied window:
```jsonc
GET /api/v1/tenants/{id}/tenders/{tender_id}/acl?limit=100&offset=0

// 200 OK
{ "entries": [ /* ACLResponse[] */ ], "limit": 100, "offset": 0 }
```
Ordered by `created_at` (unchanged); see §23.2 for why this was added and why the underlying query's index usage is unaffected.
...
### 9.5 Cache invariants

| # | Invariant |
|---|-----------|
| TAC-CACHE-1 | **There is exactly one cache region, and it is tenant-scoped.** Every key follows `tac:acl:{tenant}:{tender}:{user}` (§9.1) — unlike Core's own cache, this service has no tenant-agnostic catalog keys (no `plans`/`departments` analog) and no read-through keys sourced from another service's authoritative state; every cached value is a projection of this service's own `tender_acl_entries` table. |
| TAC-CACHE-2 | **The cache is advisory; the source of truth is this service's own Postgres.** `tac:acl:*` is a performance projection of `tender_acl_entries` (§9.2) — on any miss, timeout, or Valkey outage the service falls through to Postgres (§9.4) and returns a correct answer at degraded latency, never a wrong or stale-authoritative one. |
| TAC-CACHE-3 | **Mutations invalidate synchronously (post-commit `DEL`); the 30 s TTL provides self-healing.** Every successful TAC-2/TAC-3 write `DEL`s the affected key **after** the DB transaction commits (§9.3); if an invalidation is ever missed (process crash between commit and `DEL`), the fixed 30 s TTL bounds staleness and the entry self-heals on expiry — the same bound §9.2's passive-expiry posture (TAE-7) already tolerates. |
| TAC-CACHE-4 | **No TTL jitter.** Unlike Core's I-8 membership cache (300 s ± 30 s jitter, needed because a very large number of per-user entries can expire in the same window on a genuinely hot path), TAC-4's `tac:acl` keys carry a flat 30 s TTL with no jitter (`cache.go`'s `ttl` constant) — this service's read volume (one restricted-tender approval decision at a time, never a bulk per-user fan-out) was judged not to need stampede protection; revisit if TAC-4 traffic ever approaches I-8's shape. |
| TAC-CACHE-5 | **Fully implemented, not a partial/target-state cache.** Unlike `iam-org-membership`'s own `om:seat_usage`/`om:members` keys (invalidated but never actually read from), every `tac:acl` key this service ever `DEL`s (§9.3) is also a key some request path actually `Get`s/`Set`s (§9.2) — `ACLService.CheckAccess` is the sole reader and writer, so there is no dead half of this cache to flag as target-state. |
| TAC-CACHE-6 | **Local invalidation is post-commit, never pre-commit.** `ACLService.Grant`/`Revoke` call `cache.Delete` only after `repo.Grant`/`repo.Revoke` has already returned successfully (`internal/core/service/acl_service.go`) — a rolled-back transaction never evicts the (still-correct) cached value, and a concurrent reader can never re-populate the cache with a pre-change value between an evict and a commit that hasn't happened yet. |
| TAC-CACHE-9 | **Valkey is a performance dependency, not a correctness dependency.** A cache miss, a Valkey error, or a full outage causes a fall-through to Postgres — degraded latency only, never an outage or a wrong answer (TAC-CACHE-2). The pod stays **ready** while Postgres is healthy; `/readyz` reports the cache as *degraded* rather than failing the readiness probe (§9.4, `health.go`). |
...
|---|---|---|
| `tenant-lifecycle-tenderacl-q` — added by this extraction to replace the lost `fk_tae_tenant` `ON DELETE CASCADE` (§7.6.4), mirroring **Document 3 §7.1's identical treatment for the Group Mapping Service (decision GM-EVT-2)** | `TenantMembershipsPurged` (Core's existing tenant hard-delete/GDPR-wipe signal, unchanged event, new subscriber — renamed by `iam-org-membership` under ADR-0008 from its former name `TenantOffboarded`, specifically so it would stop colliding with Realm Provisioner's own, differently-scoped `TenantOffboarded` event; **TAC-D12**, §26) | **Hard**-delete this tenant's rows from `tender_acl_entries` (`DELETE ... WHERE tenant_id = $1`) — the replacement for the lost `ON DELETE CASCADE` to `tenants(id)` |
| `member-removal-tenderacl-q` — added after this document's v2.0 (ADR-0007 Wave 3 Phase 3, Option B — chosen over an Option A notify-client alternative), replacing the same-transaction `SoftDeleteForUser` call two independent call sites in `iam-org-membership` used to make before this table moved to its own database — `MembershipService.RemoveUser` (the tenant-facing membership-removal API) and `ProvisioningService.DeleteMember` (I-5, the Keycloak `USER_DELETE` webhook cascade; found missing its own event emission during Phase 6 prep and fixed to match) (**TAC-D10**, §26) | `MembershipRevoked` (Core's existing per-user membership-removal signal, new subscriber; from **either** producer call site above — this service's consumer treats both identically — renamed/consolidated by `iam-org-membership` under ADR-0008 from its former name `TenantMembershipRemoved`, into a single event shared with the Delegation Service's equivalent consumer; **TAC-D12**, §26) | **Soft**-delete (`deleted_at = now()`) this user's rows within the event's tenant, scoped to `(tenant_id, user_id)` — deliberately a soft delete, not the hard delete `TenantMembershipsPurged` gets, since the row itself (not just its tenant) still exists and may need to be audited later |
...
- **Idempotency:** the same `processed_events` table (§7.2.2, composite PK `(event_id, consumer)`, 8-day retention) backs both subscriptions, discriminated by `consumer` (`tenant_lifecycle_cleanup` for `TenantMembershipsPurged`, `member_removal` for `MembershipRevoked`) — a redelivered event of either type is a no-op replay.
- **DLQ:** `tenant-lifecycle-tenderacl-q-dlq` / `member-removal-tenderacl-q-dlq` respectively, both `maxReceiveCount=5`.
- **Ordering:** none required for either — both cascades are idempotent and self-contained (per tenant for `TenantMembershipsPurged`, per `(tenant, user)` for `MembershipRevoked`).
- **Envelope fields consumed:** both read the envelope's top-level `id`/`tenant_id` fields (`platform-events`' `events.Envelope[T]` — confirmed against the real published module, not assumed; `id`/`tenant_id` are top-level envelope fields, never nested under the event-specific payload/`data`); `MembershipRevoked` additionally reads the envelope's `subject` field as the removed user's ID — `iam-org-membership`'s `RemoveUser` sets `Subject` to the removed user's ID, the same convention its other subject-bearing events already use. (This service's own `processed_events.event_id` column, §7.2.2, is a locally-chosen ledger column name storing that same value — not itself a wire field name; do not conflate the two.)
...
These are the **only two** inbound events this service consumes; it publishes none (§10.3). Neither has any relationship to any authorization decision — see §7.6.3/§7.6.4 for why a delayed cleanup here is inert, not a live risk, for both cascades alike.

### 10.2 Serialization format

**JSON (UTF-8)** — the platform-wide convention (HLD §9.2), identical to every sibling service and already stated for both of §10.1's subscriptions. This service defines no serialization format of its own to choose: it consumes two event types Core already defines and serializes, via `platform-events`' `events.Envelope[T]` (plain `encoding/json`, no Protobuf/Avro) — the same consume-only posture Document 3 §7.2 documents for its own inbound-only event surface. There is no outbound serialization concern to speak of, since this service publishes nothing (§10.3, TAC-EVT-1); a future Protobuf migration, if the platform ever adopted one, would be Core's decision to make on the producer side, not this service's.
...
### 10.4 AsyncAPI contract

Like the Group Mapping Service (Document 3 §7.3), this service ships a minimal `api/asyncapi.yaml` declaring exactly two `receive` operations — `receiveTenantMembershipsPurged` on `tenant-lifecycle-tenderacl-q` and `receiveMembershipRevoked` on `member-removal-tenderacl-q` (§10.1) — and **zero** `send` operations. `schema-gov extract --check`/`validate` run against those two declarations; there is nothing for `diff`/`register` to act on since this service defines no event schema of its own for either.
...
### 10.5 AWS Glue Schema Registry and `platform-schemagov`

**Explicitly out of scope for this service**, for the same structural reason §6.1/§6.3.3 already state: there is no Glue client dependency, no schema version to register, and no entry this service owns in the Glue Schema Registry, because a service that publishes zero events has no event schema to register, diff, or version. This is stated as an explicit, documented exemption rather than a silent gap — mirroring **Document 3 §7.4's** identical treatment for the same underlying reason. The only Glue-adjacent fact relevant to this service is the receiving side of §10.4: `schema-gov validate` confirms each of this service's two `receive` declarations matches the **existing** Glue-registered schema for that event (`TenantMembershipsPurged`, `MembershipRevoked`) — a read of the registry, never a write to it.

### 10.6 Event invariants and audit
...
| TAC-EVT-5 | This service's `asyncapi.yaml` declares two `receive` operations and zero `send` operations (§10.4). |
...
Added after this document's v2.0 (**TAC-D10**, §26): when Core removes a single user's tenant membership (independent of any tenant-wide offboarding) — whether via the tenant-facing membership-removal API or the Keycloak `USER_DELETE` webhook cascade (§10.1) — it emits `MembershipRevoked` (Core's rename/consolidation, under ADR-0008, of its former `TenantMembershipRemoved`; **TAC-D12**, §26), and this service subscribes on `member-removal-tenderacl-q` (§10.1) to soft-delete that one user's rows within the affected tenant, treating both producer paths identically — replacing the same-transaction `SoftDeleteForUser` calls both of those paths in `iam-org-membership` used to make before `tender_acl_entries` moved to its own database. Like §11.5's tenant-offboarding cascade, this is an **asynchronous, eventually-consistent** cleanup with the same correctness argument: the affected user cannot present valid headers for that tenant the moment I-8 excludes them, so TAC-4 already answers `has_access:false` for their now-orphaned grants regardless of whether this cascade has run yet (§7.6.3). The one structural difference from §11.5 is the delete semantics: this cascade **soft**-deletes (`deleted_at = now()`, scoped to `(tenant_id, user_id)`) rather than hard-deleting, since only the user's membership in that tenant ended — the tenant itself, and this user's rows in any other tenant, are untouched.
...
- **The tenant-offboarding cascade-delete (§10.1, §11.5):** idempotent by construction (`DELETE ... WHERE tenant_id = $1`), backed by the `processed_events` dedup ledger (consumer `tenant_lifecycle_cleanup`). `skipDuplicate` probes the ledger before opening a write; on a hit, `tender_acl_processed_events_duplicates_total` increments and the message is acked (IDEMP-4). On a miss, the cascade and `MarkProcessed` commit in **one** `TxRunner` transaction (`withPool` / IDEMP-2, matching `iam-org-membership` / `iam-realm-provisioner`) so a crash cannot leave a committed delete unmarked. A redelivered event is still safe to reprocess even without the ledger, since a second `DELETE` against already-deleted rows is a no-op (TAC-EVT-3).
- **The per-user-removal cascade soft-delete (§10.1, §11.6):** the same `skipDuplicate` + one-transaction mark pattern under consumer `member_removal`. Idempotent by construction (`UPDATE ... SET deleted_at = now() WHERE tenant_id = $1 AND user_id = $2 AND deleted_at IS NULL`) — a redelivered `MembershipRevoked` is a no-op replay because the `deleted_at IS NULL` predicate makes a second attempt against an already-soft-deleted row match zero rows (TAC-EVT-3).
...
| A queue delivers an `event_type` neither consumer recognizes (producer rename/add, or SNS filter-policy misroute) | `tender_acl_unexpected_event_type_total{queue,event_type}` (§14.2), `TenderAclUnexpectedEventType` ticket alert after 30 min sustained | Message is acked and dropped, never retried — no cascade runs; `ackUnknown` writes a `processed_events` row so redelivery does not storm | No caller-visible effect (this event type has no downstream), but a real cascade this service *should* be running may be silently skipped until the constant is updated — this is exactly how the ADR-0008 rename bug went undetected before this metric existed (`CHANGELOG.md`) |
...
- `tender_acl_tenant_offboarding_cascade_total{result}` — the §10.1/§11.5 tenant-offboarding cascade-delete's outcome per processed `TenantMembershipsPurged` event.
- `tender_acl_member_removal_cascade_total{result}` — the §10.1/§11.6 per-user-removal cascade's outcome per processed `MembershipRevoked` event (added after this document's v2.0, **TAC-D10**, §26) — a distinct series from the tenant-offboarding one above, not a shared metric with an extra label, per TAC-EVT-6's independent-observability requirement.
- `tender_acl_unexpected_event_type_total{queue, event_type}` — incremented whenever either consumer acks-and-drops (never errors/retries) a delivery whose `event_type` doesn't match what that queue's consumer expects (§10.1/§10.6), mirroring `iam-org-membership`'s identical `iam_unknown_event_acknowledged_total`. Makes the "ignoring unexpected event type" WARN log line observable as a metric — exactly the failure mode that let both cascades silently skip-and-ack every delivery for a real stretch of time before the ADR-0008 event-type rename was caught (`CHANGELOG.md`); a sustained nonzero rate now pages via `TenderAclUnexpectedEventType` (`deploy/monitoring/app-alerts.yml`, §14.5) instead of only surfacing in logs nobody's watching.
- `tender_acl_processed_events_duplicates_total{consumer}` — incremented by `skipDuplicate` when `Envelope.ID` is already in `processed_events` (IDEMP-4), mirroring `iam-org-membership`'s identical `iam_processed_events_duplicates_total`. At-least-once SQS redelivery is observable rather than only a log line.
...
- **Postgres pool/query metrics — `pgcommon_*`** — platform-pgcommon's own instruments, activated by `pgmetrics.InitWithRegisterer` on `gincommon.MetricsRegisterer()`, identical wiring to `iam-org-membership` / `iam-realm-provisioner`. Not duplicated under a `tender_acl_*` name.
...
OTel Go SDK, W3C Trace Context, identical convention to every sibling service (HLD §12.1). An HTTP request's trace typically shows `inbound.http → service.ACLService.<List|Grant|Revoke|CheckAccess> → outbound.postgres → outbound.valkey (tac:* read/DEL)`, with an additional `outbound.membershipcheck` hop appearing only on a TAC-2 write (§7.6.2). Each of the two SQS consumers (§10.1) carries its own independent trace, rooted at `OffboardingConsumer.Handle` or `MemberRemovalConsumer.Handle` respectively, into `outbound.postgres`. Structured JSON logs — a single Zap-backed logger built via `platform-gincommon/pkg/logger.NewLogger` (identical convention to `iam-user-profile`/`iam-org-membership`, not a local slog handler; `internal/core/port.SlogStyleLogger` preserves the existing `*slog.Logger`-style call syntax on top of it), scraped to Loki — carry `trace_id`, `request_id`, `tenant_id`, and the acting principal's `sub` for every TAC-2/TAC-3 write, and `trace_id`/`tenant_id`/`event_id` (plus `user_id` for the per-user-removal cascade) for both consumers (§10.6).
...
| `tender_acl_unexpected_event_type_total` sustained nonzero for > 30 minutes | ticket, not paged — forward-compat signal (§14.2), mirroring `iam-org-membership`'s identical `IAMUnknownEventType` |
...
tenderAclConfig:
  ...
  events:
    ...
    memberRemovalQueue: member-removal-tenderacl-q   # TAC-D10, §26
    dlqMaxReceiveCount: 5             # §10.1, both queues
    processedEventsRetentionDays: 8   # §7.2.2, shared by both consumers
db:
  pool:
    maxConns: 10
    statementTimeout: 5s             # PG_STATEMENT_TIMEOUT, appended via ApplyStatementTimeout
...
`yourorg/tender-acl`, one-repo-per-microservice, using the same Clean Architecture / Ports-and-Adapters tree every sibling service uses (§6). The deliberately flat layout this section originally described (**TAC-D1**, §26) is superseded — see `CHANGELOG.md` for the restructuring commit.
...
**Migrations** run at pod startup (`migrate.Runner.Up`), additive-only to stay rolling-deploy safe (§7.4). **Graceful shutdown:** the Postgres pool's `DrainAndClose(ctx)` — the one RLS-bound app pool that also backs `processed_events` (§6.3.2, §7.2.2) — is registered before the HTTP servers/consumers, so it runs last (LIFO); each SQS consumer's own drain (both the tenant-offboarding and per-user-removal consumers, §10.1) runs before the pool closes, matching the platform-wide `main.go` ordering convention (HLD §15.3). In practice the HTTP servers' `Shutdown` and both consumers' `Stop` already wait for in-flight work to finish before the pool's deferred `DrainAndClose` call runs, so the drain itself completes immediately — `DrainAndClose` over a bare `Close` is still the correct call, as defense in depth against that ordering ever changing.
...
| Audit-log entries for TAC-2/TAC-3 writes and both cascades (tenant-offboarding, per-user-removal) | Platform general-configuration-change retention tier (§10.6) | Audit Log service |
...
Following the same classification convention as the sibling LLDs (Groups **A** — cross-document amendments, **B** — product/cross-team decisions to confirm, **C** — resolved, **D** — deferred), with items under the **TAC-Q#** prefix, distinct from this document's decision register (**TAC-D#**, §26). None of the items below blocks this document's "Approved for implementation" status.
...
**TAC-Q4 — Wave 4 entry-criteria ownership and timeline (§25) are not yet formally scheduled.** *(§25)* **Status: Proposed** — tracked as ADR-0007's own Action Item 7/8, but no owning team or target date is recorded yet.
...
Decisions already made are tracked in **§26** as **TAC-D1** through **TAC-D11** and are not duplicated here.
...
## 21. Integration Details

This service's integration surface is small by design (§3, §25): exactly **one** synchronous outbound call (to Core, TAC-2 only, §7.6.2), **two** inbound event subscriptions (from Core, §10.1), and **two** synchronous inbound HTTP consumers of its own authorization check (the Tender Service and AuthZ Enrichment, both calling TAC-4). §21.4 collates all of it in one dependency table, mirroring the sibling LLDs' own cross-service dependency tables.

### 21.1 Integration with `iam-org-membership` (Core)

| Direction | Mechanism | Description |
|---|---|---|
| This (TAC-2) → Core | HTTP `GET /internal/tenants/:id/members/:user_id/exists` (Core's I-15) | Grant-time membership-existence check, replacing the lost `fk_tae_tenant_membership` composite FK (§7.6.2) — the one synchronous outbound call this service makes, and only from TAC-2 |
| Core → (events) → This | `TenantMembershipsPurged` (`tenant-lifecycle-tenderacl-q`) / `MembershipRevoked` (`member-removal-tenderacl-q`) | Drive this service's two asynchronous cascades (§11.5/§11.6), replacing the lost `fk_tae_tenant` `ON DELETE CASCADE` and the lost same-transaction `SoftDeleteForUser` call respectively (§10.1) |

`port.MembershipCheckClient`, implemented by `internal/adapter/outbound/membershipcheck/http_client.go` (`HTTPChecker`) — the one outbound port this service has, deliberately swappable so the Wave 4 merge can repoint or delete it cheaply (§6.3.3, §7.6.2, §25).

### 21.2 Integration with the Tender Service

| Direction | Mechanism | Description |
|---|---|---|
| Tender Service (restricted-tender approval workflow) → This | HTTP `GET /internal/.../acl/:user_id` (TAC-4/I-12) | The authorization check this service exists to serve (§11.4) — the same call shape AuthZ Enrichment makes (§21.3) |
| Tender/Tenant Admin (via the Tender Service's or the platform's admin UI) → This | TAC-1/2/3 (`GET`/`POST`/`DELETE .../acl[...]`) | The grant/revoke/list admin surface (§8) |

**Not yet a real caller in this workspace** — per §24 (Migration Plan) step 2's own status note, "Tender Service" does not exist as a codebase here; this integration is written as designed, not as observed traffic, and is the reason Wave 4 (§25) remains deferred rather than executed.

### 21.3 Integration with `authz-enrichment`

| Direction | Mechanism | Description |
|---|---|---|
| AuthZ Enrichment → This | HTTP `GET /internal/.../acl/:user_id` (TAC-4/I-12) | Same call shape as §21.2 — confirmed as the one real caller present in this workspace today (§24 step 2) |

AuthZ Enrichment's own I-8 membership lookup (a separate call, to Core, not to this service) is what gates whether a user can present valid headers at all — this service's TAC-4 answer is consulted only once a request has already cleared that gate (§7.6.3).

### 21.4 Tender-ACL cross-service dependency table

Every synchronous edge this service participates in, with its budget and failure posture (mirrors the sibling LLDs' own §18.10/§18.9-style tables):

| Contract | Caller → Callee | Path | Latency budget (p99) | Posture on failure | Ref |
|---|---|---|---|---|---|
| `GET /internal/tenants/:id/members/:user_id/exists` | This (TAC-2) → Core | §21.1 | ≤50 ms | Fail-closed: Core down blocks **new grants only**; TAC-1/TAC-3/TAC-4 unaffected | §7.6.2, TAC-FAIL-1/3 |
| `GET /internal/.../acl/:user_id` (TAC-4/I-12) | Tender Service / AuthZ Enrichment → This | §21.2/§21.3 | 5 ms (cache hit) / 25 ms (cache miss) | This service down → `503`, blocks the restricted-tender approval check only | §14.1, TAC-FAIL-2 |

**No synchronous dependency touches TAC-1, TAC-3, or TAC-4** — the read path and the revoke write are pure local operations (TAC-D3). Only TAC-2, the admin grant path, makes an outbound call, and only at grant time.

## 22. Operational Considerations

### 22.1 Outbox health — not applicable

This service has no outbox (TAC-EVT-1, TAC-D5) — there is no `outbox_events` table and no `outbox.Runner` to monitor. Every table this service owns is written directly, either by TAC-2/TAC-3's own request-scoped transaction or by one of the two cascade consumers' `TxRunner` transaction (§12.2).

### 22.2 RLS violation monitoring

This service enforces RLS (§7.3) but, per **TAC-D8**, does not implement `iam-user-profile`'s forensic `rls_violation_log`/`rls_check_tenant()` SECURITY DEFINER wrapper — there is accordingly no dedicated runtime RLS-violation metric or alarm to watch here, unlike Core's own `org_membership` database. The operationally relevant gate is CI's own migration check (§7.4): every migration is verified for `rowsecurity = true AND forcerls = true` on `tender_acl_entries`, and the application role is checked for the *absence* of `BYPASSRLS` — a deploy-time gate, not a runtime alarm.

### 22.3 Grant-time membership-check dependency health (§7.6.2)

TAC-2 blocks synchronously on `port.MembershipCheckClient.Exists`; a Core outage returns `503 core_unavailable` with no partial write (TAC-FAIL-1). Watch `tender_acl_grant_checks_total{status="unavailable"}` (§14.2) alongside a spike in `503 core_unavailable` specifically — a Core dependency issue, not a fault in this service. Sustained unavailability for > 15 minutes pages at SEV-3 (§14.5); it blocks new grants only, never TAC-1/TAC-3/TAC-4 (TAC-FAIL-3).

### 22.4 Cross-service dependency & degradation matrix

| Operation | Sync dependency | Posture | On dependency failure | Ref |
|---|---|---|---|---|
| TAC-1 (list) | None | — | Pure local read; unaffected by any dependency | TAC-D3, TAC-FAIL-3 |
| TAC-2 (grant) | Core (membership-existence check) | Fail-closed | `503 core_unavailable`, no row written | §7.6.2, TAC-FAIL-1 |
| TAC-3 (revoke) | None | — | Pure local write; unaffected by any dependency | TAC-D3, TAC-FAIL-3 |
| TAC-4 (authorization check) | Valkey (performance only) | Degrade | Valkey down → fall through to Postgres, higher latency, never an outage | §9.4, TAC-CACHE-9, TAC-FAIL-2 |
| Tenant-offboarding cascade (§11.5) | None (async consumer) | Degrade | Consumer lag/DLQ depth grows; stale rows persist longer, no live authorization risk | §7.6.4, §12.3 |
| Per-user-removal cascade (§11.6) | None (async consumer) | Degrade | Same as above, scoped to one user | §7.6.3, §12.3 |

The read path (TAC-1, TAC-4) and the revoke write (TAC-3) have **no** synchronous cross-service dependency at all — mirroring TAC-D3/TAC-FAIL-3 — and only TAC-2 does, and only at grant time.

### 22.5 Tenant/member cascade fan-out health

`TenantMembershipsPurged` and `MembershipRevoked` (§10.1) drive this service's own two asynchronous cascades (§11.5/§11.6); this service is a terminal consumer of both — it relays nothing further downstream of its own (TAC-EVT-1). Each cascade runs its own `processed_events`-deduped consumer with its own DLQ (§10.1, §12.2). A consumer falling behind strands only **inert** rows belonging to an already-departed tenant or user (§7.6.3/§7.6.4) — not itself a live authorization incident, but exactly the kind of silent drift the two independent DLQ-depth alerts and `tender_acl_unexpected_event_type_total` (§14.2/§14.5) exist to surface, mirroring the observability gap that let the ADR-0008 event-rename bug go undetected for a real stretch of time before this metric existed (`CHANGELOG.md`).

## 23. Performance Considerations

### 23.1 Hot path (TAC-4)

Cache hit < 5 ms (Valkey `GET` + deserialize); cache miss < 25 ms — a single indexed lookup against `tender_acl_entries` (covered by `idx_tae_tenant_tender`/`uq_tae_active_entry`, §7.2.1), no join, no cross-service call (§9.2). The full SLO figures and how they were derived (platform-gincommon's fixed `http_request_duration_seconds` histogram bucket set) are in §14.1 and are not repeated here.

### 23.2 List endpoint (TAC-1)

**Paginated (TAC-D13, §26) — corrected from this section's own prior "unbounded and unpaginated, by design" framing, found stale during a production-readiness audit.** The result set genuinely is bounded organically by `uq_tae_active_entry`'s own scope (§7.2.1) — at most one active grant per `(tenant, tender, user)` triple, so a single restricted tender's admin listing is realistically small — but "realistically small today" is not the same guarantee as "bounded," and an unbounded response was a real production-readiness gap, not merely a theoretical one. `Handler.List` now accepts `?limit=`/`?offset=` (default limit 100, hard ceiling 500, both validated — a non-positive/non-integer `limit` or negative `offset` is `400 invalid_request`), threaded through `ACLService.List` to `TenderACLRepository.List`'s `SELECT ... WHERE tenant_id=$1 AND tender_id=$2 AND deleted_at IS NULL ORDER BY created_at LIMIT $3 OFFSET $4`, and echoed back on `ListResponse` as `limit`/`offset` so a caller can tell whether it received a full page. The query remains a single index range scan against `idx_tae_tenant_tender (tenant_id, tender_id) WHERE deleted_at IS NULL` regardless of result size — pagination bounds the response payload, not the scan cost, which was already cheap. **TAC-Q5** (§19) still tracks whether TAC-1 needs caching if its traffic grows toward TAC-4's — pagination does not resolve that question, only the unbounded-response-size one.

### 23.3 Grant/revoke write path (TAC-2/TAC-3)

Single-row writes only (§12.4) — no batch operation, no N+1 risk, no multi-table transaction to reason about beyond standard Postgres semantics. TAC-2's ≤ 100 ms p99 budget (§14.1) is dominated by the Core membership-check round trip (≤ 50 ms, §7.6.2), not by the local `INSERT`; TAC-3's ≤ 50 ms budget is a single indexed `UPDATE` with no downstream call.
...
**Execution status as of this revision — this plan is not entirely prospective; most of it has already run.** This section is the authoritative summary of that status per step (the standalone `MIGRATION_RUNBOOK.md`'s own finer-grained Phase 1–7 risk/rollback/observability/verification breakdown was removed from the repository as redundant with this section, once every phase it tracked had either completed or reached the steady "not executable in this workspace" state described below):
...
2. **Cut over — ⚠️ partially executed, not the same as complete.** Of I-12's two named callers, only `iam-authz-enrichment` is real code in this environment and has been repointed at TAC-4; **"Tender Service" does not exist yet** (§25 — this is exactly why Wave 4 is deferred) and **"admin tooling" is not a real codebase here**, so TAC-1/2/3 have nothing else to repoint. Core's P-21/22/23/I-12 handlers did return `410 Gone` for a release (Phase 5), but the tenant-offboarding event-routing repoint this step also names could **not** be executed from any code in this workspace: `TenantOffboarded` is produced by `iam-realm-provisioner`, which does not implement that emission anywhere in that repo, and the SNS subscription/filter-policy wiring that would route it to `tenant-lifecycle-tenderacl-q` is platform-Terraform-owned infrastructure, not application code. None of this has been soaked against real production traffic — no staging/production environment exists in this workspace to soak it against.
...
4. **Not part of this wave, tracked separately (Wave 4):** the eventual merge into the Tender Service — see §25.

**Rollback:** reversible through the end of step 2 by repointing TAC-1/2/3/TAC-4 callers back at Core's still-live table. Post-contract, rollback requires restoring from the pre-drop snapshot (`pg_dump --format=custom`, taken immediately before the Phase 7 drop).

---

## 25. Future Option: Moving into the Tender Service (Wave 4)
...
| TAC-D5 | No new event is introduced for ACL grant/revoke activity, preserving the source design's existing "no bus event for this concern" posture (§10.3). |
| TAC-D6 | Wave 4 (merge into the Tender Service) entry criteria are written down now (§25) rather than left implicit, per ADR-0007 Action Item 7. |
...
| TAC-D12 | **Not a new decision — this document catching up to a rename `iam-org-membership` already shipped under ADR-0008, after this document's v2.0/2.2.** The tenant-level cascade signal TAC-D7/TAC-EVT-2 describe as `TenantOffboarded` was renamed to `TenantMembershipsPurged`, specifically so it would stop colliding with Realm Provisioner's own, differently-scoped `TenantOffboarded` event — Core never re-emits RP's event, it only consumes it, and `tenant-lifecycle-tenderacl-q`'s contract was always meant to be Core's own relay, not RP's. The per-user signal TAC-D10 describes as `TenantMembershipRemoved` was consolidated into a single event, `MembershipRevoked`, shared with the Delegation Service's equivalent consumer. Both routes remain on `iam.membership.events` (never `iam.tenant.events`) on the producer side — unchanged. This service's own consumer code, and `api/asyncapi.yaml`, already shipped against the new names (`CHANGELOG.md`); this document had not been updated to match, since it lives outside the `iam-tender-acl` repo those fixes touched — the same "lives outside the repo the fix touched" gap TAC-D10/GAP-01/GAP-02 already recorded for this document. Updated to the new names: **§10.1** (event names in the queue table and mechanics bullets), **§10.4** (`receiveTenantMembershipsPurged`/`receiveMembershipRevoked` operation IDs), **§10.5** (Glue-registered schema names), **§11.6**, **§14.2** (metric descriptions), **§18.2/§18.4** (GDPR framing). TAC-D7/TAC-D10's own decision text above is left as originally written — it documents the decision that was made, not a live pointer to the wire name — matching this document's TAC-D1 precedent of adding a new entry rather than rewriting a superseded one in place. Not touched: §24's Cut-over discussion of `iam-realm-provisioner`'s *own*, still-real, still-named `TenantOffboarded` event (Discrepancy 9) — a different, unrelated event this correction does not concern. |
| TAC-D13 | Added after a production-readiness audit: TAC-1 (list) is now paginated (`?limit=`/`?offset=`, default limit 100, hard ceiling 500, both server-validated — §8.4, §23.2), correcting this document's own prior §23.2 framing that an unbounded response was acceptable "by design." The underlying query's index usage (`idx_tae_tenant_tender`) is unaffected — pagination bounds the response payload the caller receives, not the scan itself, which was already cheap. This does not resolve **TAC-Q5** (§19, whether TAC-1 needs caching as its traffic grows) — a separate question pagination does not answer. The same audit found and fixed, as plain code/tooling fixes rather than new design decisions (no register entry per each): the `membershipcheck` client silently accepting an `active:true` response missing `tenant_membership_id` instead of failing closed (§7.6.2's contract, TAC-D11, now actually enforced in code); a missing non-partial index on `tender_acl_entries(tenant_id)` that left the tenant-offboarding cascade delete (§7.6.4) unable to use an index once a tenant had any revoke history (§7.2.1); a broken Docker build (`.dockerignore` excluded `docs/` wholesale, but `cmd/tender-acl/main.go` blank-imports the generated `docs/swagger` package — every `docker build` of this service failed until this was narrowed to `docs/lld/`/`docs/architecture/` only, verified with a real build); two silent-pass holes in `release.yml`'s deploy-gate; a real HTTP smoke test added to CI (previously only image-size/fail-fast-exit checks existed); the coverage gate raised from 70% to 85% (real merged coverage was already 91.6%); and a round of stale pre-ADR-0008 event-name references (`scripts/init-localstack.sh`, Helm `values.yaml`, `.env.example`, several `.github/` files) closed the same way TAC-D12 closed them in this document. See `CHANGELOG.md` for the full itemized list. |

---

*End of document. This LLD refines ADR-0007's Wave 3 decomposition of `org_membership_lld_5.md` and is the input to implementation of the `tender-acl` service. Open items are tracked in §19; the Wave 4 merge into the Tender Service is tracked in §25.*
