...
func TestNewRouter_HealthzAndReadyz_Wired(t *testing.T) {
	...
	router := NewRouter(h, postgres, cache, nil, nil, DocsConfig{Environment: "development"})
...
func TestNewRouter_TAC4Route_Wired(t *testing.T) {
	...
	router := NewRouter(h, fakePostgresHealth{healthy: true}, fakePinger{}, nil, nil, DocsConfig{Environment: "development"})
